 ...
 *
 * Purpose:
 * - Defines WHAT queries we support (search page, count, single summary)
 * without
 * tying callers to a specific DB/SQL implementation.
 *
 * How it connects:
 * - Implemented by ListingSearchRepositoryImpl (native SQL, Postgres +
 * PostGIS).
...
public interface ListingSearchRepository {
    ...
    /**
     * Returns a page of listing "cards" (list row + marker data) that match the
     * filters.
     */
    ...
    /**
     * Returns a full card/details view for a single listing (used by the details
     * page).
     * Reuses {@link ListingCardDto} because it already contains description,
     * thumbnail,
