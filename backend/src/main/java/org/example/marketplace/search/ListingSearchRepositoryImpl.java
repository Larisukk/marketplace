...
public class ListingSearchRepositoryImpl implements ListingSearchRepository {
    ...
    WHERE (:available IS NULL OR l.available = :available)
      AND (:minPrice IS NULL OR l.price_cents >= :minPrice)
      AND (:maxPrice IS NULL OR l.price_cents <= :maxPrice)
      AND (:productId::uuid IS NULL OR l.product_id = :productId)
      ...
      AND ( :hasBbox = FALSE OR ST_Intersects(
            l.location,
    ...
    public List<ListingCardDto> search(String q, Integer minPrice, Integer maxPrice,
        ...
        // Whitelisted sort fields → translated to real column names
    ...
    """;
        ...
        return jdbc.query(sql, p, (rs, i) -> new ListingCardDto(
                ...
                rs.getString("thumbnail_url").isBlank() ? null : rs.getString("thumbnail_url")
        ));
    }
